...
import {
  User, Users, FileText, Hash, Building2, CreditCard, StickyNote, AlignLeft
} from "lucide-react";
    ...
    <ScrollArea className="h-full">
      <div className="pb-8">
        ...
        <SectionHeader icon={<User size={14} />} title="From (You)">
          ...
          <Field label="Email" value={data.fromEmail} onChange={(v) => update({ fromEmail: v })} placeholder="rahul@example.com" type="email" />
        ...
        <SectionHeader icon={<FileText size={14} />} title="Document Details">
          ...
          {data.documentType === "proposal" && (
            <>
              ...
              <Field label="Project Scope" value={data.projectScope} onChange={(v) => update({ projectScope: v })} placeholder="Describe the project scope, deliverables..." multiline rows={4} />
            </>
          )}
        </SectionHeader>
        ...
        <SectionHeader icon={<Building2 size={14} />} title="GST / Tax">
          <div className="flex items-center justify-between">
            <span className="text-xs text-zinc-400">Enable GST</span>
            <button
              onClick={() => update({ enableGST: !data.enableGST })}
              className={`relative w-10 h-5 rounded-full transition-colors ${data.enableGST ? "bg-indigo-600" : "bg-zinc-700"}`}
            >
              <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${data.enableGST ? "translate-x-5" : "translate-x-0.5"}`} />
            </button>
          </div>
          {data.enableGST && (
            <>
